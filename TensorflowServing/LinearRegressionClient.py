...
def do_inference(hostport):
  ...
  request.model_spec.name = 'linearRegression'
  request.model_spec.signature_name = 'prediction'

  # Randomly generate some test data 
  temp_data = numpy.random.randn(1000, FLAGS.numVariable).astype(numpy.float32)
  data, label = temp_data, numpy.sum(temp_data * numpy.array(numpy.arange(1, FLAGS.numVariable + 1)).astype(numpy.float32), 1)
